...
WHERE
    COMM = 'null'; 
    use Company;
   select JOB ,min(SAL) as min, max(SAL) as max from EMP group by JOB order by JOB;
select ENAME,SAL,COMM,(COMM/SAL)*100 as percentage from EMP group by ENAME having COMM is not null order by ENAME ; 
select ENAME,SAL,COMM from EMP group by ENAME having (COMM/SAL)*100 >5.0000  order by ENAME;
select JOB,count(JOB='MANAGER') as count from EMP where JOB='MANAGER';
select min(SAL) as min from EMP;
select JOB,ENAME,SAL from EMP where SAL in (select max(SAL) from EMP group by JOB )order by JOB ;
select DEPTNO,DNAME from DEPT where DEPTNO not in(select DEPTNO from EMP);
select JOB,ENAME,SAL from EMP where SAL in (select min(SAL) from EMP group by JOB )order by SAL ;